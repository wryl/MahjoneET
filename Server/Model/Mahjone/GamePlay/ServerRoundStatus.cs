...
namespace GamePlay.Server.Model
{
	public class ServerRoundStatus
	{
		...
		private List<long> playerActorNumbers;
		private IDictionary<long, string> playerNames;
		public List<bool> ChangeHandAfterRong;

		public ServerRoundStatus(GameSetting gameSettings, List<PlayerInRoom> players)
		{
			GameSettings = gameSettings;
			points = new List<int>();
			ChangeHandAfterRong = new List<bool>();
            for (int i = 0; i < gameSettings.MaxPlayer; i++)
            {
				ChangeHandAfterRong.Add(true);
			}
			playerActorNumbers = players.Select(p => p.GateSessionId).ToList();
			playerNames = players.ToDictionary(p => p.GateSessionId, p => p.NickName);
		}

        public List<Tile> GetTileFromRiver()
        {
			List<Tile> tiles = new List<Tile>();
            foreach (List<RiverTile> river in rivers)
            {
				river.ForEach(rt=> tiles.Add(rt.Tile));
				river.Clear();
			}
			return tiles;
		}
		...
		public List<long> PlayerActorNumbers => playerActorNumbers;
		...
		public List<string> PlayerNames => playerActorNumbers.Select(id => playerNames[id]).ToList();
		...
		//}
		public List<Tile> HandTiles(int index)
		{
			CheckRange(index);
		...
		public void AddToRiver(int index, Tile[] tiles, bool richi = false)
		{
			CheckRange(index);
		...
		public void RemoveFromRiver(int index)
		{
			...
			if (river.Count == 0)
			{
				Log.Warning($"There are no tiles in river {index}");
				return;
			}
		...
		public void AddKong(int index, OpenMeld kong)
		{
			...
			if (i < 0)
			{
				Log.Warning($"Pong of {kong} not found, this should not happen");
				return;
			}
		...
		public void TryRichi(int index, bool isRichiing)
		{
			...
			{
				Log.Warning($"Player {index} has already richied, therefore he cannot richi again.");
				return;
			}
		...
		public bool CheckFourWinds()
		{
			...
			if (rivers[0].Count == 0) return false;
			var first = rivers[0][0].Tile;
			for (int i = 1; i < TotalPlayers; i++)
			{
				if(rivers[i].Count==0) return false;
		...
		/// <summary>
		/// 将所有玩家牌置入牌河,并清空玩家手牌和露副.或者不清手牌
		/// </summary>
		/// <param name="playerIndex"></param>
		public int ClearOnePlayer(int playerIndex,bool changeHand)
		{
			richiStatus[playerIndex] = false;
            if (changeHand)
            {
				foreach (var tile in handTiles[playerIndex])
				{
					AddToRiver(playerIndex, tile);
				}
				handTiles[playerIndex] = new List<Tile>();
			}
			openMelds[playerIndex].ForEach(open => AddToRiver(playerIndex,open.Meld.Tiles));
			openMelds[playerIndex] = new List<OpenMeld>();
			return handTiles[playerIndex].Count;
		}
