...
namespace ET
{
    public class RoundDrawState : ServerState
	{
		...
		private M2C_RoundDrawInfo[] infos;
		private List<PointTransfer> transfers;
		private bool next;
		private bool extra;
		public void Init(RoundDrawType type)
		{
			RoundDrawType = type;
		}
		public override void OnServerStateEnter()
		{
			infos = new M2C_RoundDrawInfo[players.Count];
			...
			{
				Game.EventSystem.Publish(new EventType.ActorMessage() { actorId = players[i], actorMessage = infos[i] }).Coroutine();
			}
			TimerComponent.Instance.NewOnceTimer(ServerConstants.ServerRoundDrawTimeOut, TimeOutFunc);
		}
		...
		private void HandleRoundDraw()
		{
			...
			for (int i = 0; i < players.Count; i++)
			{
				infos[i] = new M2C_RoundDrawInfo
				{
					...
					WaitingData = waitingDataArray.ToList()
				};
			}
