...
namespace Managers
{
	...
	public static class SeId
	{
		...
		public const string Tick = "discard_tile";
		public const string Dice = "none";
		public const string Rong = "none";
	}
	#endregion
	...
	public class AudioManager : Singleton<AudioManager>
	{
		...
		[Range(0, 1)]
		[SerializeField] float _musicVolume = 0.6f;
...
//		private AudioManager() { }
		...
		private const string ActPath = @"AudioClips\ROLE\";

		private const string BgmSourceName = "bgm_{0:000}";
		private const string SeSourceName = "{0}";
		private const string ActSourceName = "{0}\\{1}";
		...
		private void Initialize()
		{
			...
			PlayBgm(BgmId.Lobby,volume:MusicVolume);
		}
		...
		public void PlaySe(string seId,  Action compCallback = null)
		{
			if (seId == SeId.None)
			{
				compCallback?.Invoke();
		...
		public void PlayAct(string seId,string rolename = "erjietang", Action compCallback = null)
		{
			if (seId == SeId.None)
			{
				compCallback?.Invoke();
			}
			var clip = Resources.Load<AudioClip>(ActPath + GetClipNameForAct(rolename,seId));
			PlaySFX(clip, clip.length, false, compCallback);
		}
		/// <summary>
		...
		/// <param name="compCallback">Action callback to be invoked after the sound has finished.</param>
		public void PlayLoopSe(string seId, float duration, Action compCallback = null)
		{
			if (seId == SeId.None)
			{
				compCallback?.Invoke();
		...
		private string GetClipNameForSe(string seId)
		{
			return string.Format(SeSourceName, seId);
		}
		private string GetClipNameForAct(string rolename,string seId)
		{
			return string.Format(ActSourceName, rolename,seId);
		}
	}
}