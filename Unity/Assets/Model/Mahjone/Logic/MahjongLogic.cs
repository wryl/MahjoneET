...
namespace Mahjong.Logic
{
	public static class MahjongLogic
	{
		...
		{
			ET.Log.Debug($"GetPointInfo method, parameters: \ndecomposes: {DecompositionToString(decomposes)}, "
			...
			foreach (var decompose in decomposes)
			{
				...
				infos.Add(info);
				ET.Log.Debug($"Decompose: {string.Join(", ", decompose)}, PointInfo: {info}");
			}

			if (infos.Count == 0) return new PointInfo();
			infos.Sort();
			ET.Log.Debug($"CountPoint: {string.Join(", ", infos.Select(info => info.ToString()))}");
		...
		public static Tile GetDoraTile(Tile doraIndicator, IList<Tile> allTiles = null)
		{
			...
			{
				ET.Log.Error($"Full tile set does not contain tile {doraIndicator}, return itself");
