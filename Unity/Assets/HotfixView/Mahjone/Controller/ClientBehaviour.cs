...
namespace GamePlay.Client.Controller
{
	...
	public class ClientBehaviourDestroySystem : DestroySystem<ClientBehaviour>
	{
		public override void Destroy(ClientBehaviour self)
		{
			self.Destroy();
		}
	}
	public class ClientBehaviour : Entity
	{
		...
		public void Awake()
		{
			...
			Managers.AudioManager.Instance.PlayBgm(Managers.BgmId.GameCommon, Managers.MusicTransition.Swift, volume: Managers.AudioManager.Instance.MusicVolume);
		}
		public void Destroy()
		{
			Debug.Log("ClientBehaviour.Destroy() is called");
			Instance = null;
			Managers.AudioManager.Instance.PlayBgm(Managers.BgmId.Lobby, volume: Managers.AudioManager.Instance.MusicVolume);
		}
		public void Rpc_BattleRoundDraw(M2C_BattleRoundDraw message)
		{
			CurrentRoundStatus.SetMahjongSetData(message.MahjongSetData);
			CurrentRoundStatus.ClearRiverData();
		}
		/// <summary>
		/// 打开选择牌的界面
		/// </summary>
		/// <param name="message"></param>
		public void Rpc_SelectTiles(M2C_SelectTiles message)
		{
			ViewController.Instance.SelectTilesPanelManager.SetTiles(message.AllTiles.ToArray());
		}
		public void Rpc_InitHandAfterRong(M2C_InitHandAfterRong message)
        {
			InitHandAfterRongState state = new InitHandAfterRongState() {
				playerIndex = message.PlayerIndex,
				MahjongSetData = message.MahjongSetData,
				CurrentRoundStatus = CurrentRoundStatus,
				LocalPlayerHandTiles = message.InitialHandTiles,
				richiStatus = message.RichiStatus.ToArray()
			};
		...
		public void RpcPreDrawTile(M2C_PreDrawTileInfo info)
		{
			var drawState = new PrePlayerDrawState
			{
				CurrentRoundStatus = CurrentRoundStatus,
		...
		public void RpcDiscardOperation(M2C_DiscardTileInfo info)
		{
		...
		public void ClientSelectTilesReady(List<Tile> tiles)
		{
			this.DomainScene().GetComponent<SessionComponent>().Session.Send(new ClientSelectTilesReady() { InitialTiles=tiles});
		}
		...
		public void OnPreDrawTile(int selectIndex)
		{
			int bonusTimeLeft = controller.TurnTimeController.StopCountDown();
			var info = new Event_SelectTileInfo
			{
				PlayerIndex = CurrentRoundStatus.LocalPlayerIndex,
				SelectIndex = selectIndex,
				BonusTurnTime = bonusTimeLeft
			};
			MahjoneHelper.OnPreDrawTile(this.DomainScene(), info);
		}
