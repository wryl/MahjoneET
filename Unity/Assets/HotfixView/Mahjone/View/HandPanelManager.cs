...
namespace GamePlay.Client.View
{
	public class HandPanelManager : MonoBehaviour, IObserver<ClientRoundStatus>
	{
		...
		[SerializeField]
		private SelectDrawTile[] SelectTiles;
		...
		private void ShowSelectDraw(ClientRoundStatus status)
		{
			var selectDraw = status.LocalSelectHandTiles;
			int length = selectDraw == null ? 0 : selectDraw.Count;

			for (int i = 0; i < length; i++)
			{
				ET.Log.Debug($"ShowSelectDraw index:{i}SelectTiles count:{SelectTiles.Length}");
				SelectTiles[i].gameObject.SetActive(true);
				SelectTiles[i].SetTile(selectDraw[i]);
			}

			for (int i = length; i < SelectTiles.Length; i++)
			{
				SelectTiles[i].gameObject.SetActive(false);
			}
		}
